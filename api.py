...
from db import DeviceBinding, License, Project, get_db
...
SIGNING_PRIVATE_KEY_PATH = Path(
    os.getenv(
        ...
        str(Path(__file__).parent / "keys" / "license_private.pem"),
    )
)


def _load_signing_private_key():
    pem = SIGNING_PRIVATE_KEY_PATH.read_bytes()
    return serialization.load_pem_private_key(pem, password=None)


_SIGNING_PRIVATE_KEY = _load_signing_private_key()
...
def _signed_response(payload: dict) -> dict:
    data = dict(payload)
    data["signature_alg"] = "RS256"
    data["signed_at"] = datetime.now(timezone.utc).isoformat()
    data["signature"] = _sign_payload(data)
    return data
...
async def verify_license(req: VerifyRequest, db: AsyncSession = Depends(get_db)):
    ...
    if not lic:
        return _signed_response({"ok": False, "error": "卡密不存在"})
    if not lic.is_active:
        return _signed_response({"ok": False, "error": "卡密已被吊销"})
    expires_at = _ensure_utc(lic.expires_at)
    if expires_at and _utc_now() > expires_at:
        return _signed_response({"ok": False, "error": "卡密已过期"})

    if req.project and lic.project_code != req.project:
        return _signed_response({"ok": False, "error": "卡密与项目不匹配"})

    result = await db.execute(select(Project).where(Project.code == lic.project_code))
    proj = result.scalar_one_or_none()
    if proj and not proj.is_active:
        return _signed_response({"ok": False, "error": "该项目已停用"})

    timeout_threshold = _utc_now() - timedelta(seconds=HEARTBEAT_TIMEOUT)
    await db.execute(
...
async def heartbeat(req: HeartbeatRequest, db: AsyncSession = Depends(get_db)):
    ...
    if not lic or not lic.is_active:
        return _signed_response({"ok": False, "error": "卡密无效或已吊销"})
    expires_at = _ensure_utc(lic.expires_at)
    if expires_at and _utc_now() > expires_at:
        return _signed_response({"ok": False, "error": "卡密已过期"})
    ...
    if not binding:
        return _signed_response({"ok": False, "error": "设备未注册，请重新启动程序"})

    binding.last_heartbeat = _utc_now()
...
async def list_projects(db: AsyncSession = Depends(get_db)):
    ...
    for project in projects:
        r = await db.execute(select(func.count()).where(License.project_code == project.code))
        license_count = r.scalar()
        r2 = await db.execute(
            select(func.count()).where(
                License.project_code == project.code, License.is_active == True
            )
        )
...
async def delete_project(code: str, db: AsyncSession = Depends(get_db)):
    ...
    if keys:
        await db.execute(delete(DeviceBinding).where(DeviceBinding.license_key.in_(keys)))
...
async def create_licenses(
    ...
    expires_at = None
    if req.expires_days:
        expires_at = _utc_now() + timedelta(days=req.expires_days)
...
async def list_licenses(
    ...
    items = []
    for lic in licenses:
        r = await db.execute(select(func.count()).where(DeviceBinding.license_key == lic.key))
...
async def revoke_license(req: RevokeLicenseRequest, db: AsyncSession = Depends(get_db)):
    ...
    lic.is_active = False
    await db.execute(delete(DeviceBinding).where(DeviceBinding.license_key == req.license_key))
    await db.commit()
        ...
        items.append(
            {
                ...
                "is_online": device.last_heartbeat > timeout_threshold,
            }
        )
